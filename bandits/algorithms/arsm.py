...
class ARSMSampler(models.Model):
  ...
  def action(self, inputs):
    """1. Sample a true action"""
    omega = self.sampler.sample()
    log_omega = tf.math.log(omega)
    a_true = tf.math.argmin(log_omega - inputs)
    return a_true
...
# class ARSM(BayesianNN):
#   """Implements ARSM layer on top of neural network for bandit problems."""
...
#       x, num_units, 
#       activation=self.haparas.activation,
#       weights_initializer=tf.random_uniform_initializer(-init_s, init_s))

#     # if dropout:
#     #   # nn = tf.nn.dropout(nn, self.hparams.keep_prob)
#     #   nn = tf.keras.layers.Dropout(nn, self.hparams.keep_prob)

#     # Put the ARSM code here!
